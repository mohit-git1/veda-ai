...
export default function LibraryPage() {
  ...
  const { assignments, removeAssignment } = useAssignmentStore()
    ...
    if (activeTab === 'Saved') {
      return (
        <div className="flex flex-col items-center justify-center py-24 text-center">
          ...
          <button onClick={() => router.push('/assignments')} className="bg-[#111111] text-white rounded-full px-6 py-3 text-sm font-medium hover:bg-gray-800 transition-colors shadow-sm active:scale-95">Browse Assignments</button>
        </div>
      )
    }
          ...
          <div>
            ...
            {completedPapers.length > 0 ? (
                ...
                {completedPapers.map(p => (
                  <div key={p._id} className="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm hover:shadow-md transition-shadow">
                    <div className="flex items-start gap-4 mb-5">
                      <div className="w-12 h-12 rounded-full bg-orange-50 text-[#E8431C] flex items-center justify-center flex-shrink-0">
                      ...
                      <button onClick={() => removeAssignment(p._id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg transition-colors">
              ...
              <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-3xl border border-gray-100 shadow-sm">
                ...
                <button onClick={() => router.push('/assignments/create')} className="bg-[#111111] text-white rounded-full px-6 py-3 text-sm font-medium hover:bg-gray-800 transition-colors shadow-sm active:scale-95">Create Assignment</button>
              </div>
            )}
          </div>
        )}
